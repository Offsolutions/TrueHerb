...
public partial class Auth_Report : System.Web.UI.Page
{
    ...
    public static int pair = 0, pairincome = 0;
    ...
    {
        string id = Common.Get(objsql.GetSingleValue("select updatepin from usersnew where regno='" + Session["user"] + "'"));
        // pair = Convert.ToInt32(Common.Get(objsql.GetSingleValue("select pintype from pins where pin='"+id+"'")));
        if (pair == 1950)
        {
            pairincome = 300;
        }
    ...
    protected void gvpins_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            if ((left >= 2 && right >= 1) || (left >= 1 && right >= 2))
            {
                if (left < right)
                {
                    amount.Text = (left * pairincome).ToString();
                }
                else if (left == right)
                {
                    left = left - 1;
                ...
                {
                    amount.Text = (right * pairincome).ToString();
                }
            ...
            else
            {
                if (left == right)
                {
                    ...
                    {
                        amount.Text = (pairincome * minus).ToString();
                    }
                }
                else if (left < right)
                {
